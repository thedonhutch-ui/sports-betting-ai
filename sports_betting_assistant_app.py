...
def guess_team_column(df, teams):
    for col in df.columns:
        values = df[col].astype(str).str.lower()
        if any(t.lower() in values.values for t in teams):
            return col
    return None
...
if odds_df.empty:
    ...
    if not df.empty:
        ...
        if stats_df.empty:
            ...
            guessed_col = guess_team_column(stats_df, selected_teams)

            if guessed_col is None:
                st.warning("Couldn't auto-detect team column. Please choose manually.")
                guessed_col = st.selectbox("Select column containing team names", stats_df.columns)

            stats_df[guessed_col] = stats_df[guessed_col].astype(str).str.lower()
            selected_teams_lower = [t.lower() for t in selected_teams]
            team_stats = stats_df[stats_df[guessed_col].isin(selected_teams_lower)]

            if not team_stats.empty:
                st.dataframe(team_stats)

                melted = team_stats.melt(id_vars=guessed_col, var_name="Stat", value_name="Value")
                chart = alt.Chart(melted).mark_bar().encode(
                    x=alt.X('Stat:N', title="Stat"),
                    y=alt.Y('Value:Q'),
                    color=f'{guessed_col}:N',
                    column=f'{guessed_col}:N'
                ).properties(width=150)
                st.altair_chart(chart, use_container_width=True)

                st.download_button(
                    "\ud83d\udcbe Download Picks + Stats",
                    data=picks_df.merge(team_stats, left_on="Pick", right_on=guessed_col).to_csv(index=False),
                    file_name="value_picks_with_stats.csv",
                    mime="text/csv"
                )
            else:
                st.info("No matching stats found for selected picks.")
