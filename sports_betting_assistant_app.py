...
def fetch_team_stats(gid):
    try:
        ...
        df.columns = df.columns.str.strip()  # Clean any hidden whitespace
        return df
    except Exception as e:
        st.error(f"Error loading stats from Google Sheets: {e}")
        return pd.DataFrame()
    ...
    if not df.empty:
        ...
        if stats_df.empty:
            ...
            if "Team" not in stats_df.columns:
                st.error("🛑 Google Sheet must contain a 'Team' column.")
            else:
