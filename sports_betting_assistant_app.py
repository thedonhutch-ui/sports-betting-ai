...
if odds_df.empty:
    ...
    if not filtered_df.empty:
        ...
        st.subheader("Team Stats (From CSV Upload)")
        uploaded_stats = st.file_uploader("Upload Team Stats CSV", type="csv")

        if uploaded_stats:
            try:
                stats = pd.read_csv(uploaded_stats)
                ...
                st.error(f"Error loading uploaded stats: {e}")
